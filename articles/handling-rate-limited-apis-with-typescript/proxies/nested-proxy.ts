...
function createNestedProxy<T extends object>(root: T, bucket: TokenBucket): T {
  ...
  function createProxy<U extends object>(target: U): U {
    if (cache.has(target)) {
      return cache.get(target) as U;
    }

    console.log("creating a proxy for", target);
    ...
    const proxy = new Proxy(target, {
      get(target, prop, receiver) {
        ...
        if (
          typeof reflectedProperty === "object" &&
        ...
        // check if the property is a function
          ...
          return (...args: unknown[]) =>
            tryConsume(bucket, reflectedProperty.bind(target, ...args));
        } else {
...
async function withNestedProxy() {
  ...
  // adding a few more calls
  proxiedApi.users.getUsers();
  proxiedApi.users.getUsers();
  proxiedApi.users.getUsers();
  proxiedApi.users.getUsers();
